...
export function CollectionStatus({ count, loading }: CollectionStatusProps) {
  if (loading) {
    return (
      <FadeIn>
  ...
  // Ne rien afficher si pas de collections
  if (count === 0) {
    return null
  }
    ...
    <FadeIn>
      <div className="flex items-center justify-center gap-4 mb-8 flex-wrap">
        <Badge variant="outline" className="bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800 text-green-700 dark:text-green-300 px-4 py-2">
          ...
          {count} collection{count > 1 ? 's' : ''} disponible{count > 1 ? 's' : ''}
        </Badge>
      </div>
    </FadeIn>
  )
